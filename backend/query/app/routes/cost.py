    ...
    try:
        ...
        for row in rows:
            ...
            data.append({
                ...
                'request_count': request_count,
                'cost_per_request': round(float(row['cost_per_request']), 6),
                'cost_per_1k_tokens': round(cost_per_1k_tokens, 4),
                'total_tokens': row['total_tokens'],
                'avg_latency_ms': 0,  # Placeholder - would need to join with performance data
                'error_rate': round((row['error_count'] / request_count * 100), 2) if request_count > 0 else 0,
        ...
        result = {
            ...
            'meta': {
                ...
                'total_cost_all_agents': round(sum(d['total_cost'] for d in data), 2)
            }
        }
...
async def get_cost_attribution(
    ...
    try:
        ...
        departments = {}
        total_cost = float(sum(row['cost'] for row in rows))
        ...
        for dept in departments.values():
            dept['percentage'] = round((dept['value'] / total_cost_float) * 100, 2) if total_cost_float > 0 else 0
            dept['children'] = list(dept['children'].values())
            for agent in dept['children']:
                agent['percentage'] = round((agent['value'] / total_cost_float) * 100, 2) if total_cost_float > 0 else 0
    ...
    try:
        ...
        query = """
            SELECT
                ...
                0 as tokens_cached,
                ROUND(
                    (0::numeric / NULLIF(SUM(tokens_input + tokens_output), 0)) * 100, 
                    2
                ) as cache_hit_rate,
                SUM(CASE 
                    WHEN tokens_input > 0 THEN cost_usd * (tokens_input::numeric / NULLIF(tokens_input + tokens_output, 0))
                    ELSE 0 
                END) as input_cost,
                SUM(CASE 
                    WHEN tokens_output > 0 THEN cost_usd * (tokens_output::numeric / NULLIF(tokens_input + tokens_output, 0))
                    ELSE 0 
                END) as output_cost,
                SUM(CASE 
                    WHEN 1=0 THEN cost_usd * 0.1
                    ELSE 0 
                END) as cached_cost
            FROM traces
            WHERE workspace_id = $1
        ...
        total_input = int(row['total_tokens_input'] or 0)
        total_output = int(row['total_tokens_output'] or 0)
...
async def get_cost_forecast(
    ...
    try:
        ...
        for i in builtin_range(1, 31):
            next_date = last_date + timedelta(days=i)
            predicted_cost = max(0, costs[-1] + (slope * i))

            forecast.append({
                'date': next_date.isoformat() if hasattr(next_date, 'isoformat') else str(next_date),
...
async def get_caching_roi(
    ...
    try:
        ...
        query = """
            SELECT
                COUNT(*)::int as total_requests,
                SUM(CASE WHEN 1=0 THEN 1 ELSE 0 END)::int as cache_hits,
                SUM(CASE WHEN 1=1 THEN 1 ELSE 0 END)::int as cache_misses,
                SUM(cost_usd) as cost_with_cache,
                AVG(CASE WHEN 1=0 THEN latency_ms ELSE NULL END) as avg_cached_latency,
                AVG(CASE WHEN 1=1 THEN latency_ms ELSE NULL END) as avg_uncached_latency
            FROM traces
            WHERE workspace_id = $1
                AND timestamp >= NOW() - INTERVAL '1 hour' * $2
        """
...
async def get_agent_token_efficiency(
    ...
    Returns:
    - avg_tokens_per_call, avg_tokens_input, avg_tokens_output
    ...
    try:
        ...
        query = """
            SELECT
                AVG(tokens_input + tokens_output)::numeric as avg_tokens_per_call,
                AVG(tokens_input)::numeric as avg_tokens_input,
                AVG(tokens_output)::numeric as avg_tokens_output,
                ROUND((0::numeric / NULLIF(SUM(tokens_input + tokens_output), 0)) * 100, 2) as cache_hit_rate,
                (SUM(cost_usd) / NULLIF(SUM(tokens_input + tokens_output), 0) * 1000)::numeric as cost_per_1k_tokens
            FROM traces
            WHERE workspace_id = $1
                AND agent_id = $2
                AND timestamp >= NOW() - INTERVAL '1 hour' * $3
        ...
        trend_query = """
            SELECT
                ...
                AVG(tokens_input + tokens_output)::numeric as tokens_per_call,
                (SUM(cost_usd) / NULLIF(SUM(tokens_input + tokens_output), 0))::numeric as cost_per_token
            FROM traces
            WHERE workspace_id = $1
                AND agent_id = $2
                AND timestamp >= NOW() - INTERVAL '1 hour' * $3
        ...
        async with pool.acquire() as conn:
            ...
            result = {
                ...
                'avg_input_tokens': float(row['avg_tokens_input'] or 0),
                'avg_output_tokens': float(row['avg_tokens_output'] or 0),
